# 科研资源与工作流导航

更新时间：2026-04-16

本仓库用于整合科研工作流中最常用的公开资源，包括 `Skills`、`MCP`、agent 系统、文献与知识工具、实验框架、benchmark、写作与投稿工具，以及 `AI/ML`、`data mining`、`computational design science` 等方向专题。整体采用“任务优先、方向补充、类型附录查全”的组织方式；`scientific discovery` 相关内容保留为补充支线，而不是主轴。

## 快速开始

第一次进入，建议先看：

1. [00-总览/快速开始](./00-总览/快速开始.md)
2. [00-总览/任务与资源对照表](./00-总览/任务与资源对照表.md)
3. [09-按科研任务导航](./09-按科研任务导航/README.md)
4. [08-方向专题](./08-方向专题/README.md)

## 高频入口

- 想补实验链路、benchmark、agent eval： [05-实验执行、复现与评测](./05-实验执行、复现与评测/README.md)
- 想做 novelty 判断、综述、review mining： [04-新颖性判断与综述](./04-新颖性判断与综述/README.md)
- 想系统扫 `Skills / MCP`： [98-全局索引/Skills索引](./98-全局索引/Skills索引.md) / [98-全局索引/MCP索引](./98-全局索引/MCP索引.md)
- 想补实验工具、CLI、benchmark： [98-全局索引/实验与Benchmark索引](./98-全局索引/实验与Benchmark索引.md) / [98-全局索引/CLI与终端工具索引](./98-全局索引/CLI与终端工具索引.md)
- 想补 AI/ML 与 data mining 子方向： [08-方向专题/AI-ML-DataMining资源](./08-方向专题/AI-ML-DataMining资源.md)

## 热门组合

- `选题 -> novelty -> 实验 -> 写作`：
  [09-按科研任务导航/课题扫描与选题](./09-按科研任务导航/课题扫描与选题.md) ->
  [09-按科研任务导航/新颖性判断与综述](./09-按科研任务导航/新颖性判断与综述.md) ->
  [09-按科研任务导航/实验执行、复现与评测](./09-按科研任务导航/实验执行、复现与评测.md) ->
  [09-按科研任务导航/写作、审稿与投稿](./09-按科研任务导航/写作、审稿与投稿.md)
- `精读 -> 知识沉淀 -> 综述`：
  [09-按科研任务导航/文献检索与单篇精读](./09-按科研任务导航/文献检索与单篇精读.md) ->
  [09-按科研任务导航/知识管理与证据沉淀](./09-按科研任务导航/知识管理与证据沉淀.md) ->
  [09-按科研任务导航/新颖性判断与综述](./09-按科研任务导航/新颖性判断与综述.md)
- `查 Skills / MCP / benchmark`：
  [91-Skills](./91-Skills/README.md) ->
  [92-MCP](./92-MCP/README.md) ->
  [98-全局索引](./98-全局索引/README.md)

## 按角色进入

- 做选题与方向扫描：
  [09-按科研任务导航/课题扫描与选题](./09-按科研任务导航/课题扫描与选题.md)
- 做文献精读与知识沉淀：
  [09-按科研任务导航/文献检索与单篇精读](./09-按科研任务导航/文献检索与单篇精读.md) /
  [09-按科研任务导航/知识管理与证据沉淀](./09-按科研任务导航/知识管理与证据沉淀.md)
- 做 novelty 判断与 reviewer 预演：
  [09-按科研任务导航/新颖性判断与综述](./09-按科研任务导航/新颖性判断与综述.md)
- 做实验、复现与 benchmark：
  [09-按科研任务导航/实验执行、复现与评测](./09-按科研任务导航/实验执行、复现与评测.md)
- 做成稿、返修与投稿：
  [09-按科研任务导航/写作、审稿与投稿](./09-按科研任务导航/写作、审稿与投稿.md)

## 按科研任务进入

- [01-课题扫描与选题](./01-课题扫描与选题/README.md)
- [02-文献检索与单篇精读](./02-文献检索与单篇精读/README.md)
- [03-知识管理与证据沉淀](./03-知识管理与证据沉淀/README.md)
- [04-新颖性判断与综述](./04-新颖性判断与综述/README.md)
- [05-实验执行、复现与评测](./05-实验执行、复现与评测/README.md)
- [06-写作、审稿与投稿](./06-写作、审稿与投稿/README.md)

## 横向专题与治理

- [07-安全与治理](./07-安全与治理/README.md)
- [08-方向专题](./08-方向专题/README.md)
- [09-按科研任务导航](./09-按科研任务导航/README.md)

## 类型附录与全局索引

- [90-按类型参考](./90-按类型参考/README.md)
- [91-Skills](./91-Skills/README.md)
- [92-MCP](./92-MCP/README.md)
- [93-科研工作流与Agent](./93-科研工作流与Agent/README.md)
- [94-Prompt与写作](./94-Prompt与写作/README.md)
- [95-文献检索与知识管理](./95-文献检索与知识管理/README.md)
- [96-实验与评测](./96-实验与评测/README.md)
- [97-模板与投稿](./97-模板与投稿/README.md)
- [98-全局索引](./98-全局索引/README.md)

常用快捷索引：
[98-全局索引/Skills索引](./98-全局索引/Skills索引.md)、
[98-全局索引/MCP索引](./98-全局索引/MCP索引.md)、
[98-全局索引/文献与知识索引](./98-全局索引/文献与知识索引.md)、
[98-全局索引/实验与Benchmark索引](./98-全局索引/实验与Benchmark索引.md)、
[98-全局索引/写作与投稿索引](./98-全局索引/写作与投稿索引.md)

## 推荐使用方式

### 方式 A：按任务直接进入

适合当前已经知道自己处在“选题、精读、综述、实验、投稿”哪一步的场景。
优先入口：[09-按科研任务导航](./09-按科研任务导航/README.md)

也可以先看：[00-总览/任务与资源对照表](./00-总览/任务与资源对照表.md)

### 方式 B：按研究方向进入

适合 AI/ML、data mining、computational design science、开放创新等方向的专题补充。
优先入口：[08-方向专题](./08-方向专题/README.md)

### 方式 C：按资源类型查全

适合需要系统扫 `Skills / MCP / agent / benchmark / prompt / 模板` 的场景。
优先入口：[90-按类型参考](./90-按类型参考/README.md)

## 组织原则

- 顶层主线优先围绕科研任务展开，便于直接查询和使用。
- `07-09` 负责横向治理、方向专题与任务使用导航。
- `91-97` 保留为查全型附录，适合系统扫库、补库和后续扩展。
- 同一外部资源的较完整说明尽量只保留在一个主归属页，其余页面以短说明和内链引用为主。
- 中文说明优先，收录对象以中英文公开资源并重，不限制资源原始语言。

## 维护入口

- [00-总览/统一收录模板](./00-总览/统一收录模板.md)
- [00-总览/补库规则与待补清单](./00-总览/补库规则与待补清单.md)
- [00-总览/更新日志](./00-总览/更新日志.md)
